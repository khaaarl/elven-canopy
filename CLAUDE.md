...
- **Shared crates + game crates.** `elven_canopy_prng` provides a hand-rolled xoshiro256++ PRNG used by all crates (no external RNG dependencies). `elven_canopy_lang` provides shared Vaelith conlang types, vocabulary (JSON lexicon), and name generation — used by both `elven_canopy_sim` (elf names) and `elven_canopy_music` (lyrics). `elven_canopy_utils` provides shared utilities: deterministic fixed-point arithmetic (`Fixed64` scalar, `FixedVec3` 3D vector with 2^30 fractional bits) used by both `elven_canopy_sim` (projectile ballistics) and `elven_canopy_music` (composition scoring), plus parallel dedup algorithms. `elven_canopy_sim` is a pure Rust library (zero Godot dependencies) containing all simulation logic. `elven_canopy_graphics` contains chunk mesh generation, smoothing, decimation, and texture generation — depends on sim for voxel types but is otherwise independent. `elven_canopy_gdext` is a thin wrapper that exposes the sim and graphics to Godot via GDExtension. `elven_canopy_music` is a standalone Palestrina-style polyphonic music generator with Vaelith (elvish) lyrics. The sim/gdext separation is enforced at the compiler level; the music crate is independent of both.
...
- **Voxel world, voxel-direct pathfinding.** The world is a 3D voxel grid (sim truth). Ground creatures pathfind with A* directly on the voxel grid using walkability predicates and edge-type classification. Flying creatures use A* on the open 3D voxel grid with footprint clearance.
...
Individual crate tests: `cargo test -p elven_canopy_sim`, `cargo test -p elven_canopy_graphics`, `cargo test -p elven_canopy_lang`, `cargo test -p elven_canopy_music`, `cargo test -p tabulosity -p tabulosity_derive`. Tabulosity serde tests: `cargo test -p tabulosity --features serde --test serde`. Music CLI: `cargo run -p elven_canopy_music -- --help`.

**Targeted clean:** If stale build artifacts cause errors (e.g., after force-pushes or branch switches), clean only the project crates — not the entire target directory: `cargo clean -p elven_canopy_sim -p elven_canopy_graphics -p elven_canopy_gdext`. This preserves the cached `godot` crate build, which is very slow to recompile.
...
- **Determinism (sim crate)**: `elven_canopy_sim` must produce identical results given the same seed. No hash-order dependence, no set iteration, no floating-point arithmetic (precision varies across platforms/compilers), no stdlib PRNG. All crates share a hand-rolled xoshiro256++ PRNG from `elven_canopy_prng` (with SplitMix64 seeding) — no external PRNG crate dependencies. This enables consistency in multiplayer and verification of optimizations. **Scope:** The strict determinism constraint (identical results across platforms/compilers) applies to `elven_canopy_sim`. The music crate uses the same PRNG for seed-based reproducibility but doesn't participate in lockstep multiplayer or replay verification. **What determinism means (CRITICAL — read carefully):** Determinism is a *same-source-code* guarantee: two instances built from the **same source code, regardless of optimization parameters or OS**, starting from the same state must produce identical results. It does NOT mean that PRNG call sequences must be preserved across different releases. Changing the number of PRNG calls in a new release is perfectly fine — it just means a given seed will produce different world details than before, which is expected and acceptable. Do not treat PRNG call-count changes as breaking determinism. Saves store state snapshots, not PRNG replay logs.
...
4. **Use `fresh_test_seed()` for new tests, never hardcoded seeds.** New tests must call `fresh_test_seed()` (defined in `test_helpers.rs`) to get a random seed that is printed on failure for reproducibility. Do not use `legacy_test_seed()`, `test_sim(42)`, or any hardcoded numeric seed — these exist only for pre-existing tests that haven't been hardened yet. If a test using `fresh_test_seed()` flakes, that means the test needs better robustness (explicit positioning, `force_guaranteed_hits`, `zero_creature_stats`, etc.), not a hardcoded seed. Prefer `flat_world_sim(fresh_test_seed())` over `test_sim(fresh_test_seed())` unless the test genuinely needs tree geometry.
