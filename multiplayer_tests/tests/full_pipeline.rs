...
/// Relay-level pause/resume: turns stop flowing when paused, resume when unpaused.
#[test]
fn relay_pause_resume() {
    let (handle, mut host, mut joiner, _addr) = start_test_session();
    start_game(&mut host, &mut joiner);

    // Host pauses the relay.
    host.send_pause();
    host.poll_until_paused();
    joiner.poll_until_paused();

    // While paused, drain briefly — no new turns should arrive.
    let tick_before = host.sim.as_ref().unwrap().tick;
    thread::sleep(Duration::from_millis(200));
    let (turns_h, _) = host.drain_turns();
    let (turns_j, _) = joiner.drain_turns();
    assert_eq!(turns_h, 0, "no turns should arrive while paused (host)");
    assert_eq!(turns_j, 0, "no turns should arrive while paused (joiner)");
    assert_eq!(
        host.sim.as_ref().unwrap().tick,
        tick_before,
    ...
    // Turns should start flowing again.
    thread::sleep(Duration::from_millis(200));
    let (turns_h, _) = host.drain_turns();
    let (turns_j, _) = joiner.drain_turns();
    assert!(
        turns_h > 0,
        "turns should resume after unpause (host got {turns_h})"
    );
    assert!(
        turns_j > 0,
        "turns should resume after unpause (joiner got {turns_j})"
    );
