...
#[test]
fn inv_move_stack_clears_equipped_slot() {
    // An equipped item moved to another inventory must have equipped_slot
    // cleared — equip state is meaningless outside the creature's inventory.
    let mut sim = test_sim(42);
    let src = sim.create_inventory(crate::db::InventoryOwnerKind::Creature);
    let dst = sim.create_inventory(crate::db::InventoryOwnerKind::Structure);
    sim.inv_add_item(
    ...
    let stack_id = sim
        ...
        .by_inventory_id(&src, tabulosity::QueryOpts::ASC)[0]
        .id;
    // Verify it starts equipped.
    assert_eq!(
        sim.db.item_stacks.get(&stack_id).unwrap().equipped_slot,
        Some(inventory::EquipSlot::Head),
    );
    ...
    assert_eq!(
        ...
        "equipped_slot must be cleared on inter-inventory transfer"
    );
}

#[test]
fn inv_merge_clears_equipped_slot() {
    // inv_merge moves all stacks from src to dst; equipped_slot must be
    // cleared on every transferred stack.
    let mut sim = test_sim(42);
    let src = sim.create_inventory(crate::db::InventoryOwnerKind::Creature);
    let dst = sim.create_inventory(crate::db::InventoryOwnerKind::Structure);
    sim.inv_add_item(
    ...
    for stack in &dst_stacks {
        assert_eq!(
            stack.equipped_slot, None,
            "{:?} should not be equipped after inv_merge",
            stack.kind,
        );
    }
}
...
#[test]
fn death_clears_ownership_on_remote_items() {
    // Items owned by a creature but located in a remote ground pile (e.g.,
    // dropped during a prior task interruption) must have ownership cleared
    ...
    // Create a remote ground pile with items owned by this elf.
    let remote_pos = VoxelCoord::new(elf_pos.x + 10, elf_pos.y, elf_pos.z + 10);
    let pile_id = sim.ensure_ground_pile(remote_pos);
    let pile_inv = sim.db.ground_piles.get(&pile_id).unwrap().inventory_id;
    sim.inv_add_simple_item(pile_inv, inventory::ItemKind::Bread, 5, Some(elf_id), None);

    // Verify the items are owned before death.
    let stacks_before = sim
        .db
        .item_stacks
        .by_inventory_id(&pile_inv, tabulosity::QueryOpts::ASC);
    assert_eq!(stacks_before[0].owner, Some(elf_id));

    // Kill the elf.
    let mut events = Vec::new();
    sim.handle_creature_death(elf_id, crate::types::DeathCause::Debug, &mut events);

    // The remote pile's items should now be unowned.
    let stacks_after = sim
        .db
        .item_stacks
        .by_inventory_id(&pile_inv, tabulosity::QueryOpts::ASC);
    assert!(
        !stacks_after.is_empty(),
        "Remote pile should still have items"
    );
    for stack in &stacks_after {
        assert_eq!(
            stack.owner, None,
            "Remote items owned by dead creature must have ownership cleared"
        );
    }
}

#[test]
fn death_drops_equipped_items_unequipped() {
    // When a creature dies wearing equipped clothing, the dropped items
    // in the ground pile must have equipped_slot cleared. This covers
    // both owned items and unowned military equipment (which enters
    // the creature's inventory via AcquireMilitaryEquipment without
    // setting ownership).
    let mut sim = test_sim(42);
    sim.config.elf_starting_bread = 0;
    sim.config.elf_starting_bows = 0;
    ...
    // Equipped hat owned by the elf (personal clothing).
    sim.inv_add_item(
        creature_inv,
        inventory::ItemKind::Hat,
        1,
    ...
    // Equipped tunic with NO owner (military equipment path).
    sim.inv_add_item(
        creature_inv,
        inventory::ItemKind::Tunic,
        1,
    ...
    // Find items in ground piles. The pile may be snapped to the surface
    // below the elf's walking height, so search all piles.
    let pile_stacks: Vec<_> = sim
        .db
        .ground_piles
        .iter_all()
    ...
    let hat = pile_stacks
        .iter()
        .find(|s| s.kind == inventory::ItemKind::Hat)
        .expect("Hat should be in ground pile");
    assert_eq!(
        hat.equipped_slot, None,
        "Dropped hat must not retain equipped_slot"
    );

    let tunic = pile_stacks
        .iter()
        .find(|s| s.kind == inventory::ItemKind::Tunic)
        .expect("Tunic should be in ground pile");
    assert_eq!(
        tunic.equipped_slot, None,
        "Dropped tunic must not retain equipped_slot"
    );
}

#[test]
fn death_clears_ownership_on_building_inventory_items() {
    // Items owned by a creature but located in a building inventory must
    // have ownership cleared when the creature dies.
    let mut sim = test_sim(42);
    sim.config.elf_starting_bread = 0;
    sim.config.elf_starting_bows = 0;
    let elf_id = spawn_elf(&mut sim);
    ...
    // Create a building and put items owned by the elf in it.
    let building_anchor = VoxelCoord::new(tree_pos.x + 3, tree_pos.y, tree_pos.z);
    let sid = insert_building(&mut sim, building_anchor, Some(5), Vec::new());
    let building_inv = sim.db.structures.get(&sid).unwrap().inventory_id;
    sim.inv_add_simple_item(
        building_inv,
        inventory::ItemKind::Bread,
        3,
    ...
    // The building's items should now be unowned.
    let stacks = sim
        .db
        .item_stacks
        .by_inventory_id(&building_inv, tabulosity::QueryOpts::ASC);
    assert!(!stacks.is_empty(), "Building should still have items");
    for stack in &stacks {
        assert_eq!(
            stack.owner, None,
            "Building items owned by dead creature must have ownership cleared"
        );
    }
}

#[test]
fn death_sweep_does_not_clear_other_creatures_ownership() {
    // Two creatures each own items in the same ground pile. When creature A
    // dies, creature B's items must retain their ownership.
    let mut sim = test_sim(42);
    sim.config.elf_starting_bread = 0;
    sim.config.elf_starting_bows = 0;
    ...
    // Create a ground pile with items owned by both elves.
    let pile_id = sim.ensure_ground_pile(elf_a_pos);
    let pile_inv = sim.db.ground_piles.get(&pile_id).unwrap().inventory_id;
    sim.inv_add_simple_item(pile_inv, inventory::ItemKind::Bread, 3, Some(elf_a), None);
    sim.inv_add_item(
        pile_inv,
        inventory::ItemKind::Arrow,
        5,
    ...
    // Kill elf A.
    let mut events = Vec::new();
    sim.handle_creature_death(elf_a, crate::types::DeathCause::Debug, &mut events);

    // Elf A's bread should be unowned.
    let stacks = sim
        .db
        .item_stacks
        .by_inventory_id(&pile_inv, tabulosity::QueryOpts::ASC);
    let bread = stacks
        .iter()
        .find(|s| s.kind == inventory::ItemKind::Bread)
        .expect("Bread should still be in pile");
    assert_eq!(bread.owner, None, "Dead elf's bread must be unowned");

    // Elf B's arrows must retain ownership.
    let arrows = stacks
        .iter()
        .find(|s| s.kind == inventory::ItemKind::Arrow)
        .expect("Arrows should still be in pile");
    assert_eq!(
        arrows.owner,
        Some(elf_b),
        "Living elf's arrows must retain ownership"
    );
}
