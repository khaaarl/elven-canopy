...
fn fp_harmonic_mean(a: i64, b: i64) -> i64 {
    ...
    // 2 * a * b / (a + b), careful with overflow:
    // Compute (a * b) >> FP_SHIFT first, then 2 * that / sum * FP_ONE.
    // More precisely: 2 * a/sum * b (avoids intermediate overflow).
    let ab = (a as i128 * b as i128) >> FP_SHIFT;
    (2 * ab / sum as i128 * FP_ONE as i128 / (1i128 << FP_SHIFT)) as i64
}
