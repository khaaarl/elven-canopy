...
- **Task claiming eligibility:** Creatures with mp_max = 0 (nonmagical) cannot claim mana-requiring tasks at all — construction requires magically singing things into existence. For magical creatures, the eligibility check has two parts: (1) the creature has enough mana reserves to complete at least several work actions immediately, and (2) the creature's mana generation rate can sustain the task's expenditure rate over time. Both must be satisfied. (Exact formula and heuristics for very large projects that exceed any single elf's capacity are future work.)
