...
Mana exists at two levels — **creature mana** and **tree mana** — forming a layered economy.

#### Creature Mana

- Each creature has a current and maximum mana pool (mp / mp_max). Many creatures are nonmagical (mp_max = 0).
- Magical creatures (elves) generate mana over time into their personal pool, at a base rate modified by happiness (initially flat-rate; mood-dependent scaling in F-mana-mood).
- Creatures spend personal mana when performing magical work — construction singing, and eventually Grow-verb crafting recipes.
- When a creature's mana exceeds its personal capacity, the excess flows to the tree it is bonded to (via shared civilization). Overflow is checked each creature heartbeat.
- If a creature's mana is below its maximum, a blue mana bar is shown above its sprite, positioned below the HP bar (same billboard pattern, offset downward by one bar height). Creatures with non-zero mp_max also show a mana bar on their creature info panel.

#### Tree Mana

- Trees have mana storage (mana_stored / mana_capacity) but do not generate mana on their own. Trees accumulate mana from their bonded elves' overflow.
- Tree mana serves as a communal reserve. Future mechanics (greenhouses, magical structures, tree-to-elf mana transfer) will draw from or interact with the tree pool.
- The tree info panel displays the tree's current mana and capacity.

#### Mana and Work

- Construction and furnishing tasks (and eventually Grow-verb crafting) have a per-action mana cost. Each work action (one voxel placed, one furniture item installed) drains a fixed amount of mana from the working creature at the start of the action. The drain amount is determined by the build type and specified in `GameConfig` — currently `platform_mana_cost_per_voxel` and `bridge_mana_cost_per_voxel`. Build types without a specific config field (stairs, walls, struts, carving, ladders, furnishing) use a `default_mana_cost_per_action` fallback. ("Per action" is the general term covering both voxel placement and furniture installation.)
- If a creature lacks sufficient mana for a work action, it still spends the time performing the action but accomplishes no work (no progress, no voxel placed). This is a wasted action, not a skip — the creature is visibly trying but failing.
- A creature tracks consecutive wasted actions in a `wasted_action_count` field (reset to 0 on any successful work action or task change, including mope interrupts). When `wasted_action_count` reaches a configurable threshold (`mana_abandon_threshold` in `GameConfig`), the creature abandons the task. The task reverts to `Available` state with all progress preserved (voxels already placed remain); another elf with sufficient mana can pick it up and continue.
- **Task claiming eligibility:** Creatures with mp_max = 0 (nonmagical) cannot claim mana-requiring tasks at all — construction requires magically singing things into existence. For magical creatures, the current implementation requires enough mana for at least one work action to claim a task. A more sophisticated rate-based sustainability estimate (generation rate vs. expenditure rate) and heuristics for very large projects are future work.

#### Mana Overflow

- Each creature heartbeat, after mana generation, if the creature's mp exceeds mp_max, the excess is transferred to the bonded tree (the tree owned by the creature's civilization). The creature's mp is clamped to mp_max.
- If the tree's mana_stored is already at mana_capacity, the excess is lost (no further accumulation). The tree does not exceed its capacity.
- Creatures with no civilization bond (civ_id = None, e.g., wild creatures) have no overflow target — their excess mana is simply lost.
...
> **Note:** The current implementation uses individual elf task assignment (one elf claims a Build task, walks to the site, and works solo). The choir-based group construction described below is the aspirational design for F-choir-build; it is not yet implemented.
