...
`db.update_creature_with(&id, |c| c.hunger += 10)` avoids the
clone-modify-update dance. The closure mutates the row in place and
indexes are updated afterward. Low complexity.
