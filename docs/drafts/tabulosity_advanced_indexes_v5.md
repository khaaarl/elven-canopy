# Tabulosity — Compound and Filtered Indexes (Design Draft v5)

This is v5 of the compound/filtered indexes design. Key changes from v4:
`QueryBound::Any` renamed to `QueryBound::MatchAll` (matching the public unit
struct name), `PartialEq`/`Eq` added to `QueryBound` derives, PK field in
`fields(...)` now triggers a compile-time error, type canonicalization
restricted to simple types and `Option<T>`, various documentation
clarifications (IDE limitation for filter paths, stale bounds in long-running
sessions, `.clone()` uniformity in codegen, `in_bounds` fully-qualified path,
owned vs reference asymmetry in query API, simple index rewrite scope note).
...
- **Unified query API.** One query method per index, with each field parameter
  accepting exact values, ranges, or `MatchAll` via an `IntoQuery` trait. No
...
- **Leftmost prefix semantics.** Same mental model as database compound indexes:
  leading exact/range fields use the index efficiently; once `MatchAll` appears,
...
range bounds. For example, a prefix query `by_foo(&a, MatchAll)` on a
...
Where `{type_name}` is derived from the type as written in the struct field
definition (see "Type Name Canonicalization" below). The naming scheme ensures
that if multiple fields or indexes use the same type, they share one tracked
bounds pair.
...
- **Tracked bounds never shrink.** They only grow outward ("ever seen"
  ...
  the removed row held the current min or max. See "Stale Bounds Safety" below
...
- **When tracked bounds are `None`** (no rows ever inserted), queries that need
  bounds (prefix scans, `MatchAll`) return empty immediately — there is nothing
  in the index.

### Stale Bounds Safety

After deletions, tracked bounds may be wider than the actual data range. This
is by design.

- **Wider bounds never exclude valid results.** They may cause the BTreeSet
  range scan to visit a few empty nodes at the edges, which is negligible.
- **Bounds are recomputed to exact values on `rebuild_indexes()`**
  (deserialization), so staleness does not accumulate across save/load cycles.
- **The alternative** — recomputing bounds on every delete by scanning all
  values — would be O(n) per delete and is not worth the precision.
- **In long-running sessions with heavy churn**, tracked bounds grow
  monotonically. This has zero performance impact — wider BTreeSet range scans
  don't visit extra entries because the BTreeSet itself doesn't contain values
  outside the actual data range. For applications that want tighter bounds,
  `rebuild_indexes()` can be called manually to recompute bounds from current
  data.
...
the `_bounds_{type_name}` field. Compound index fields are restricted to
**simple types** and **`Option<T>`** where T is a simple type:

- **Simple types:** any path without generic parameters (e.g., `u8`, `String`,
  `TaskId`, `crate::TaskId`).
- **Option types:** `Option<SimpleType>` (e.g., `Option<CreatureId>`,
  `Option<String>`).

Types like `Result<A, B>`, `Vec<T>`, `Option<Option<T>>`, or any other
compound generic are **not supported** in compound index fields. The macro
emits a compile-time error if it encounters an unsupported generic type. This
restriction simplifies the canonicalization rules and avoids edge cases with
deeply nested or multi-parameter generics.

The canonicalization rules for supported types are:

- The macro uses the type **exactly as written** in the struct field definition.
- Two fields with the same written type share one tracked bounds pair.
- The field name is derived by converting the type's **last path segment** to
  snake_case, with generic parameters appended. Examples:
  - `u8` -> `_bounds_u8`
  - `TaskId` -> `_bounds_task_id`
  - `Option<CreatureId>` -> `_bounds_option_creature_id`
  - `String` -> `_bounds_string`
- Path prefixes like `crate::`, `self::`, `super::` are stripped before
  sanitization. So `crate::TaskId` and `TaskId` share bounds.
- If two different types happen to produce the same sanitized name (e.g., a
  newtype `OptionCreatureId` and `Option<CreatureId>`), the macro emits a
  **compile-time error** about the collision.
- Type aliases (`type Foo = Bar`) are undetectable by the proc macro and will
  create redundant tracking — this is acceptable and documented as a known
  limitation.

The proc macro collects all unique types across all index fields and the PK,
deduplicates them by sanitized name (erroring on collisions from distinct
types), and generates one `_bounds_*` field per unique type.
...
- `fields(...)` (required): ordered list of field names forming the composite
  ...
  fields with a filter (compound + filtered). The primary key field must NOT
  appear in this list (see "Compile-Time Error Handling" below).
...
**Single-field `#[index]` without filter:** A declaration like
`#[index(name = "foo", fields("bar"))]` with one field and no filter is valid.
It is semantically identical to `#[indexed]` on that field — just a more
verbose way to write the same thing. No special rejection or warning is needed.
...
**Breaking change:** This replaces the existing simple index code paths. The
following items from the current API are removed (not deprecated):

- The `_range` method variants (`by_{field}_range`, `iter_by_{field}_range`,
  `count_by_{field}_range`) are removed. The unified `IntoQuery`-based API
  subsumes them — `by_assignee(3..7)` replaces `by_assignee_range(3..7)`.
- The `map_start_bound` / `map_end_bound` helper functions are removed.
- The `Bounded` trait is no longer required on PK types or indexed field types.

Since nothing outside the tabulosity test suite depends on the old API, this
is a clean break. The tabulosity tests will be updated to use the new API.

**Implementation note:** This is a major implementation effort — it requires
rewriting the entire existing simple index codegen path (currently in
`tabulosity_derive/src/table.rs`), not just adding new code alongside it. The
old `map_start_bound`/`map_end_bound` helpers and `Bounded` requirement on PK
are removed entirely, and the query method generation is replaced with the
unified `IntoQuery`-based codegen described in this document.
...
`QueryBound` derives `Clone` because the `iter_by_*` generated code needs to
clone `QueryBound` values into `move` closures. Since all field types appearing
in indexes are required to be `Clone`, and `Bound<T>` implements `Clone` when
`T: Clone`, the derive works for all valid field types. `Debug` is derived for
convenience in error messages and test assertions. `PartialEq` and `Eq` are
derived for use in testing — they enable direct comparison of `QueryBound`
values in assertions (e.g.,
`assert_eq!((&42).into_query(), QueryBound::Exact(42))`).
...
// MatchAll and `..` (RangeFull) are equivalent.
...
// For example, (Bound::Excluded(a), Bound::Excluded(b)) has no corresponding
// named range type. This impl lets users pass any (start, end) bound pair.
...
table.by_species_hunger(Species::Elf..Species::Capybara, MatchAll)

// All rows in the index (useful for filtered indexes — returns only matching rows).
table.by_active_priority(MatchAll, MatchAll)

// Skip on first field, exact on second — works but O(n) post-filter.
table.by_species_hunger(MatchAll, &30)

// Arbitrary bound combination — both-excluded range on second field.
// No named Rust range type covers this case.
use std::ops::Bound;
table.by_species_hunger(&Species::Elf, (Bound::Excluded(10), Bound::Excluded(40)))

// Half-open range with an excluded start bound.
table.by_species_hunger(&Species::Elf, (Bound::Excluded(5), Bound::Included(20)))
```

**Owned values in ranges vs `&T` for exact:** Exact matches use `&T`
(reference, cloned internally by the `IntoQuery` impl), while ranges use owned
values (e.g., `3..7` not `&3..&7`). This follows Rust's standard range types
which own their bounds — `Range<T>` contains `start: T` and `end: T`, not
references.

`RangeFull` (`..`) also works anywhere `MatchAll` does, since both map to
`QueryBound::MatchAll`. The `MatchAll` struct exists for readability —
`table.by_foo(MatchAll, &3)` reads more clearly than `table.by_foo(.., &3)`.
...
impl TaskTable {
    ...
    /// Returns a boxed iterator over references to matching rows (borrows table).
    pub fn iter_by_foo(
        &self,
        a: impl IntoQuery<AType>,
        b: impl IntoQuery<BType>,
    ) -> Box<dyn Iterator<Item = &Task> + '_> { ... }
...
**`iter_by_*` returns `Box<dyn Iterator>`:** The generated query code dispatches
on the `QueryBound` variants via a `match` statement. Different match arms
produce different concrete iterator types (e.g., a BTreeSet `range()` iterator
with a `.map()` vs. a BTreeSet `iter()` with a `.filter().map()`). These
concrete types cannot unify under `impl Iterator` because the return type must
be a single concrete type across all arms. Boxing resolves this: each match arm
boxes its iterator chain, and the method returns `Box<dyn Iterator<Item = &Row>
+ '_>`. The `+ '_` lifetime ensures the borrow of `self` is captured by the
trait object. This is one heap allocation per query — negligible overhead — and
preserves the borrowing benefit (the iterator borrows `self`, so callers don't
need the whole `Vec` in memory).

The `by_*` (returning `Vec`) and `count_by_*` (returning `usize`) methods are
unaffected since they call `.collect()` / `.count()` inside each match arm,
consuming the iterator immediately. No boxing needed.
...
depends on which fields have concrete constraints (Exact or Range) vs.
`MatchAll`:
...
- **Once `MatchAll` appears**, that field and all subsequent fields are
  unconstrained in the index scan. If later fields have concrete constraints,
  they become post-filters applied to each row after retrieval.
...
| `by_foo(&a, MatchAll)`          | Exact       | MatchAll    | Prefix scan, O(log n + k)     |
| `by_foo(1..5, &b)`              | Range       | Exact       | Range scan + post-filter      |
| `by_foo(1..5, MatchAll)`        | Range       | MatchAll    | Range scan, O(log n + k)      |
| `by_foo(MatchAll, &b)`          | MatchAll    | Exact       | Full index scan + post-filter |
| `by_foo(MatchAll, MatchAll)`    | MatchAll    | MatchAll    | Full index scan               |
...
of the composite tuple. **A Range or MatchAll field** contributes its bounds (or
tracked min/max for `MatchAll` and `Unbounded`). **Trailing fields after
`MatchAll`** use tracked min/max for the index scan, then results are
post-filtered.
...
Example — `by_foo(&some_assignee, MatchAll)` (prefix query):
...
Example — `by_foo(MatchAll, &3)` (non-leading exact — post-filter):
...
### Range Followed by Non-MatchAll

When a Range field is followed by a non-`MatchAll` field, the trailing field
cannot narrow the index scan — it becomes a post-filter. This is because the
BTreeSet range covers all values of the range field, and within each value, all
...
- `MatchAll` fields: start wrapper is `Included` (with tracked_min), end wrapper is
...
start: Included((a.clone(), 3, pk_min.clone()))  // A=a exact, B=3 inclusive
end:   Excluded((a.clone(), 7, pk_min.clone()))  // A=a exact, B=7 excluded, pad PK with tracked_min
```
...
start: Included((a.clone(), 3, pk_min.clone()))  // A=a exact, B=3 inclusive
end:   Included((a.clone(), 7, pk_max.clone()))  // A=a exact, B=7 inclusive, pad PK with tracked_max
```

### Helper Functions

The existing `map_start_bound` and `map_end_bound` helpers (which require
`PK: Bounded`) are removed when this is implemented. They are replaced by
...
This will live in `tabulosity/src/table.rs`, replacing the removed
`map_start_bound` / `map_end_bound` helpers. Generated code calls it via the
fully-qualified path `::tabulosity::in_bounds(...)` to avoid shadowing any
user-defined `in_bounds` function in the calling module.

### Generated Query Code (Sketch)

For `#[index(name = "active_priority", fields("assignee", "priority"), filter = "Task::is_active")]`:

**`by_*` method (returns `Vec`):**
...
pub fn by_active_priority(
    ...
    match (&a, &b) {
        ...
        // Exact + MatchAll — prefix scan
        (QueryBound::Exact(a_val), QueryBound::MatchAll) => {
        ...
        // MatchAll on first field — scan everything, post-filter second field
        (QueryBound::MatchAll, _) => {
            self.idx_active_priority
                .iter()
                .filter(|(_, b_val, _)| match &b {
                    QueryBound::Exact(v) => b_val == v,
                    QueryBound::Range { start, end } => ::tabulosity::in_bounds(b_val, start, end),
                    QueryBound::MatchAll => true,
                })
                .map(|(_, _, pk)| self.rows[pk].clone())
                .collect()
        }
...
**`iter_by_*` method (returns `Box<dyn Iterator>`):**

The `iter_by_*` method follows the same match structure, but each arm boxes its
iterator chain instead of calling `.collect()`:

```rust
pub fn iter_by_active_priority(
    &self,
    assignee: impl IntoQuery<Option<CreatureId>>,
    priority: impl IntoQuery<u8>,
) -> Box<dyn Iterator<Item = &Task> + '_> {
    ...
    match (&a, &b) {
        ...
        // MatchAll on first field — scan everything, post-filter second field
        (QueryBound::MatchAll, _) => {
            let b = b.clone();  // clone into move closure
            Box::new(
                self.idx_active_priority
                    .iter()
                    .filter(move |(_, b_val, _)| match &b {
        ...
        // ... other arms follow the same pattern: box the iterator chain.
        // Each arm returns Box::new(...) wrapping the specific iterator type.
    }
}
```

Each arm returns `Box::new(...)` wrapping its specific iterator chain. The
`move` keyword on filter closures is mandatory for `iter_by_*` because the
iterator outlives the function scope — the closure must take ownership of the
resolved `QueryBound` values. This requires `QueryBound<T>: Clone` so that
values matched by reference in the outer `match` can be cloned into the `move`
closure. For `by_*` (returning `Vec`), `move` is technically optional since
`.collect()` consumes the iterator immediately.
...
- **Position 0 (first field is Range or MatchAll):** Range/full scan on field 0,
  post-filter fields 1..N.
- **Position 1 (field 0 Exact, field 1 is Range or MatchAll):** Lock field 0 in
  bounds, Range/full scan on field 1, post-filter fields 2..N.
- **Position k (fields 0..k-1 Exact, field k is Range or MatchAll):** Lock fields
...
maintenance (insert, update, upsert) would leave the table in an inconsistent
state — for example, the row may have been inserted into the BTreeMap but
indexes not yet updated, or some indexes updated but not others. This is the
same panic-safety situation that exists for any code maintaining derived data
structures: if a derived update panics partway through, the base and derived
data are out of sync. It is not unique to filtered indexes.

This is consistent with a general principle: **all user-provided functions
called by tabulosity must be infallible.** Currently the only user-provided
functions are filter predicates, but the principle extends to any future
callback mechanisms (e.g., computed columns, custom validators). If a filter
function can fail, the failure must be handled internally (e.g., by returning
`false` as the default) rather than by panicking.
    ...
    (true, true) => {
        ...
        if old_row.assignee != new_row.assignee || old_row.priority != new_row.priority {
            self.idx_active_priority.remove(&(
                ...
                old_row.priority.clone(),
                pk.clone(),
            ));
            self.idx_active_priority.insert((
                new_row.assignee.clone(),
                new_row.priority.clone(),
                pk.clone(),
            ));
        }
    }
...
NOTHING in the index machinery depends on it. The `map_start_bound` /
`map_end_bound` helpers and the `_range` method variants are removed. The
`Bounded` trait and `derive(Bounded)` remain available but are no longer used
by any generated code.

### On the Filter Function

- Must be `fn(&Row) -> bool` — a bare function, not a closure.
- **Must not panic.** A panic during index maintenance leaves the table in an
  inconsistent state (see "Panic safety" in the Filtered Index Maintenance
  section above).
...
- **IDE limitation:** Because the filter path is a string literal in an
  attribute, it does not get IDE support — no go-to-definition, no rename
  refactoring, no find-usages. If the function is renamed, the attribute breaks
  at compile time (not silently), so the failure mode is loud. This is the same
  limitation as serde's `#[serde(deserialize_with = "...")]`.
...
- **PK field in `fields`:** If a field listed in `fields(...)` is the
  `#[primary_key]` field, the macro emits: "field 'id' is the primary key and
  is automatically included in every index; remove it from fields(...)". The PK
  is automatically appended as the trailing tiebreaker in every index tuple —
  including it explicitly would duplicate it, wasting space and producing
  confusing semantics.
...
- **Tracked bounds name collision:** Two distinct types that produce the same
  sanitized `_bounds_{type_name}` identifier (see "Type Name Canonicalization"
  above).
- **Unsupported generic type in index field:** A compound index field with a
  type that is not a simple type or `Option<SimpleType>` (e.g., `Vec<T>`,
  `Result<A, B>`, `Option<Option<T>>`). See "Type Name Canonicalization" above.
...
contribute to the composite tuple bounds. Once `MatchAll` or Range-followed-by-
non-`MatchAll` is encountered, pad trailing fields with
`tracked_min`/`tracked_max` and switch to post-filtering for remaining fields.
...
- **Bounded trait removal.** Once all generated code uses tracked bounds, the
  `Bounded` trait, `derive(Bounded)`, and their blanket impls could be removed
  in a future major version if no external code depends on them. They remain
  for now as utility types.
