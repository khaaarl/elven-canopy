# Tabulosity — Compound and Filtered Indexes (Design Draft v3)

This is v3 of the compound/filtered indexes design. Key changes from v2:
`iter_by_*` methods return `Box<dyn Iterator>` (needed because match arms
produce different concrete iterator types), the `Any` unit struct is renamed to
`MatchAll` (avoids collision with `std::any::Any`), type name canonicalization
rules are fully specified, and stale tracked bounds behavior is explicitly
documented.
...
pub fn iter_by_active_priority(
    ...
    match (&a, &b) {
        ...
        // Any on first field — scan everything, post-filter second field
        (QueryBound::Any, _) => {
            let b = b.clone();  // move into closure
            Box::new(
                self.idx_active_priority
                    .iter()
                    .filter(move |(_, b_val, _)| match &b {
...
resolved `QueryBound` values. For `by_*` (returning `Vec`), `move` is
technically optional since `.collect()` consumes the iterator immediately.
