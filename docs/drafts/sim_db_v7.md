# Tabulosity — In-Memory Relational Store (Design Draft v7)
...
Changes from v6: foreign key declarations now live ONLY on the Database struct
(via the `fks(...)` syntax on `#[table]` attributes), eliminating the DRY
violation where FK info was declared in two places; the Table derive no longer
has a `#[foreign_key]` attribute — FK fields are just `#[indexed]`; introduced a
`FkCheck` helper trait that lets the Database derive generate uniform FK
validation code for both bare and `Option<FK>` fields; `update_*` now validates
ALL FK fields (not just changed ones) for simplicity and correctness; added
`Debug` to required PK trait bounds; clarified `table.rs` contents (no generic
`Table<K, V>` type).
...
for how this connects to the `creatures` table.
...
Note: the restrict-on-delete code does still need to know whether the FK field
is `Option<T>` or bare `T`, because the index stores the field's actual type.
For `Option<CreatureId>`, the index stores `(Option<CreatureId>, TaskId)` tuples,
so the restrict check must query `count_by_assignee(&Some(id.clone()))`. For
bare `CreatureId`, the index stores `(CreatureId, TaskId)` tuples, so the check
is `count_by_assignee(id)`. The Database derive doesn't know which case applies
(it can't see the row struct), so it generates `count_by_assignee(id)` and
relies on the Rust compiler to flag a type mismatch if the field is actually
`Option`. In practice, the user writes the correct type in the `fks`
declaration or the compiler catches it. (Alternatively, a future refinement
could add syntax like `fks(assignee? = "creatures")` to explicitly mark optional
FKs in the Database attribute, but this is not part of v7.)
...
- **Optional FK syntax in Database attribute.** Something like
  `fks(assignee? = "creatures")` to explicitly mark optional FK fields, enabling
  the Database derive to generate correct restrict-on-delete code without
  relying on Rust compiler errors for type mismatches. Low priority — the
  current approach works and compiler errors are clear.
