# Tabulosity — Compound and Filtered Indexes (Design Draft v4)

This is v4 of the compound/filtered indexes design. Key changes from v3:
`QueryBound` derives `Clone` (and `Debug`), `IntoQuery` impl for
`(Bound<T>, Bound<T>)` tuple covering arbitrary bound combinations, filter
function panic safety documented, and terminology fixes (`MatchAll` used
consistently in user-facing prose instead of `Any`).
...
the `_bounds_{type_name}` field. The rules are:
...
Note: The `QueryBound::Any` variant keeps the name `Any` — it is an internal
enum variant used by generated code, not a name that users type. The
user-facing unit struct is `MatchAll`.
...
convenience in error messages and test assertions.
...
`QueryBound::Any`. The `MatchAll` struct exists for readability —
...
tracked min/max for `Any` and `Unbounded`). **Trailing fields after `MatchAll`**
use tracked min/max for the index scan, then results are post-filtered.
...
`map_start_bound` / `map_end_bound` helpers.
